...
**语义兜底（route-guard 词表不中时同样适用）：** 用户描述"筛一遍这堆需求"、"要不要先过一遍再进 brainstorm"这类批量需求预筛意图 → `/muse-req-triage`；"从需求到原型跑一遍完整流程/闭环"这类端到端自治编排意图 → `/muse-loop-orchestrate`。
...
/ 工程链 / Plan Agent）/ 本文件声明的工具动作，据此路由，不等用户说对触发词。分寸闸双向：① STOP /
...
**侧栏当前页感知（claude 拉取，2026-07-11；依赖 luca app 运行中）：** 用户说"看看我侧栏/当前打开的页面/
...
FxTwitter（`api.fxtwitter.com/<handle>/status/<id>` 无 key 恢复全文）；
本地 HTML 预览页签 → 直接 Read meta 给的本地路径；登录墙/动态页无法重取 →
`bash scripts/luca-sidebar.sh capture` 抓 DOM 正文兜底。
③ 下游接轨：诉求为"评估纳入 skill os / 工作流" → 接 `external-skill-scout`（Workflow）；评估
通过要采纳 → 走既有治理轨道（main 落地 + routing-map + /office + workflow-graph +
