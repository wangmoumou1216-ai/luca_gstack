...
| **Plan Agent 层** | route-guard 输出 `PLAN MODE`（复杂度分 ≥ 6）**或** route-guard 输出 `PLAN CHECK`（命中重型编排器 skill：`/deepresearch`、`/ux-research`、`/figma-demo`——**不含 `/auto`**，2026-07-03 起 `/auto` 靠自己 Step 2 的内部 Phase 确认门，不再叠加外部 PLAN_CHECK，见「重型 skill」表）**或** 命中 skill 已知满足 Plan Agent 5条件之一 | 读取 `.claude/agents/plan-agent.md`，输出 Phase 计划，等用户确认后进入 Orchestrator |
...
handoff/adr/engineering/evaluation/evals/retro/redteam 等按 skill 产出分子目录，
`PROGRESS.md`/`handoff/` 见上方 Context 工程协议）。
