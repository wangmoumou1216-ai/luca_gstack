...
>   session 已保留）"；此时首条消息 route-guard 会额外提示"本 session 继承了激活项目 X，
>   非所需请 switch"（防静默写错项目）——**看到该提示且你要做的不是项目 X 时，务必先
>   `./scripts/project.sh switch`，不要直接开工**。Meta/审计/内容工具 skill 例外分支不变。
