...
>   session 已保留）"；此时首条消息 route-guard 提示"当前激活 X（并行保留）"。**命名即切换
>   （2026-07-06）后无需手动 switch**：你一提别的项目名 / 语义上描述别的项目或新项目，主 Agent
>   就按 Project Gate ②③ **自动切换/新建**（新建会 detach 当前）；只有消息完全不指向任何项目、
>   又要在这个从未确认过的继承项目上做实质任务时，才一句话确认再动手。Meta/审计/内容工具 skill
>   例外分支不变。
>   **并行安全（轻量替代，非 per-session 隔离）：** 激活项目仍是全局共享软链，切走会覆盖并行
>   session 的指针；但**不再静默**——被切走的 session 下一轮 route-guard 会告警"本 session 原在 X，
>   当前激活已被切到 Y"，且其写 `docs/` 时 post-edit 当场点名"可能落错项目"。即"命名即切换 + 防误写
>   告警"，不是物理隔离（真隔离需 per-session worktree，评估过量重，未做）。
