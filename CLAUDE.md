...
a project, and before any project-scoped read/write/switch/create or cross-project reference.
<!-- K5:END -->
...
harness invoked the other's mechanism. Before answering, cross-harness work loads
`.claude/skill-os/runtime/cross-harness.md` and must verify both independently. Runtime
safety/capability facts bind, while behavioural preferences do not erase semantic routing; on such
a conflict load `.claude/skill-os/runtime/harness-boundary.md`. Select agent capability from
`.claude/skill-os/model-routing.yaml`; inherit when unsure and never hardcode a model name.
<!-- K10:END -->
