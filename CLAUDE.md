...
This is the independent Claude Code entry contract for **luca_gstack**. It contains only the
always-on safety and routing kernel. Detailed truth lives in the one-hop owners named below; read a
target directly and through its final line when its condition matches. Do not load `AGENTS.md`.
Conditional loading is behavioral: read each matching target through EOF by its `load_before`
boundary; citing it unread afterward is noncompliant.
...
Ambiguity requires one user choice; no match requires catalog discovery. In discovery, **name the exact matching catalog skill** and authority; a generic “catalog is discoverable” is incomplete.
<!-- K4:END -->
...
project reference. Naming, implying, switching, creating, or cross-reading a project reaches it. A
framework/meta/`NO_PIN` explanation making no such decision or project I/O uses the inline NO_PIN
floor without reading the cold owner.
<!-- K5:END -->
...
`.claude/skill-os/model-routing.yaml` before selecting, validating, or dispatching capability;
inherit when unsure and never hardcode a model name.
<!-- K10:END -->
