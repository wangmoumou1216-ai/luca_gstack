...
| **Plan Agent 层** | route-guard 输出 `PLAN MODE`（复杂度分 ≥ 6）**或** route-guard 输出 `PLAN CHECK`（命中重型编排器 skill：`/deepresearch`、`/ux-research`、`/auto`、`/figma-demo`）**或** 命中 skill 已知满足 Plan Agent 5条件之一 | 读取 `.claude/agents/plan-agent.md`，输出 Phase 计划，等用户确认后进入 Orchestrator |
