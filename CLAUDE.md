...
| `/open-design` | A B C D | **Open Design 产出（设计产出首选）**：编译需求成 OD 指令 → 评估并让你选 platform+design system → 绑定建项目 → headless 一次性生成 → 回收落盘 + figma-layer；FxUI 只叠品牌色+文字色，其余走所选 design system |
| `magicpath` | A B C D | MagicPath 界面产出（备选，OD 不可达时）：需求描述 → React canvas 组件 |
...
| **Plan Agent 层** | route-guard 输出 `PLAN MODE`（复杂度分 ≥ 6）**或** route-guard 输出 `PLAN CHECK`（命中重型编排器 skill：`/deepresearch`、`/ux-research`、`/auto`、`/figma-demo`）**或** 命中 skill 已知满足 Plan Agent 4条件之一 | 读取 `.claude/agents/plan-agent.md`，输出 Phase 计划，等用户确认后进入 Orchestrator |
...
- 「全流程」「自动做」「auto」「一键」「完整设计」「全套功能设计」「完整功能设计」
  「端到端」「全套方案」「帮我全部」「CRM/商机/客户管理 + 设计类词」→ `/auto`（多
  skill 组合时优先于单个 skill）
- 「会议纪要」「语音稿/转文字稿」「原始语料」「讨论记录转需求」「整理这段记录」→ `/idea`
  （仅限"已有原始语料忠实结构化"；新想法 / 方向探索 / 需求梳理一律不走 /idea）
- 「写 PRD」「产品需求」「brainstorm」「需求文档」「需求梳理」「需求分析」「需求确认」
  「有个想法」「原始想法」「功能方向」「一个功能」→ `/brainstorm`
...
- 「竞品分析」「UX研究」「设计调研」「用户研究」「体验分析」「UX调研」→ `/ux-research`
- 「看看竞品」「截图」「竞品截图」「浏览网站」→ `agent-browser`（快速直观浏览，非完整研究）
...
> **重要：** `session-restore.mjs` 在每次 SessionStart 会主动清除三个 symlink：
> `docs/` → 项目 docs、`.claude/workflow-state.yaml` → 项目 state、
> `.claude/current-topic.txt` → 项目 topic。这是有意的设计（git log: `feat:
> auto-deactivate project on every startup`），确保每次新 session 都走"确认项目
> 归属"流程，防止跨 session 状态污染。**后果：** 每次 session 启动都呈"无激活项目"
> 状态，第一条用户消息会触发 Project Gate 询问"新项目还是继续老项目"，除非
> 命中 Meta/审计任务或内容工具 skill 例外（见 route-guard.mjs projectGate
> Audit C2/M2 例外分支）。
