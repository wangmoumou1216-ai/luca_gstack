...
纯框架自建 session **每月建议 ≤2 次**（软上限，自省提示非硬门禁；红线/CI 红/安全修复不受限；
响应式改进优先于预防式重构）；治理产出批处理优先、默认攒批到季度裁决。依据与全文：appendix。
...
经验沉淀不依赖用户开口提醒，三环节自动闭环：**捕获**（Stop hook 拦截"有实质工作未沉淀"
的 session 就地裁决；三重防循环 + fail-open）→ **治理+晋升**（每日首 session 后台跑
`daily_governance.py`：只晋升 promotion_ready 门禁内候选（红线 SC-20260523-003）、降频
digest（有变化才写 + 每 7 天强制心跳）、「⚙️ Loop 健康」自检）→ **回看**（下次启动提示
最新 digest）。机制细节全文：`.claude/skill-os/claude-md-appendix.md`。
...
| **Plan Agent 层** | route-guard 输出 `PLAN MODE`（复杂度分 ≥ 6，**关键词近似判定**）**或** `PLAN CHECK`（命中 `HEAVY_ORCHESTRATOR_SKILLS` 扩展点——**母版默认空**，2026-07-04 起 deepresearch/ux-research/figma-demo/auto 全部靠各自内部 HITL 门，见 plan-agent.md「条件 2 豁免」；该 set 保留为 fork/env 扩展点）**或** 命中 skill 满足 Plan Agent 5 条件之一（**`.claude/agents/plan-agent.md` 触发条件表是唯一权威口径**；PLAN MODE 是其关键词近似、研究默认门 = 5 条件 + 新颖，均从属于它） | 读取 `.claude/agents/plan-agent.md`，输出 Phase 计划，等用户确认后进入 Orchestrator |
...
自判、侧栏感知等）统一为**一条**通用反射——**每次请求开始、以及执行途中冒出新子目标 / 范围变化 / 新的
设计或工程需求时**，我都以「含义」判断该请求是否映射到某 skill / 某流程（设计链 / 工程链 / Plan Agent）
/ 本文件声明的工具动作，据此路由，不等用户说对触发词。分寸闸双向：① STOP / 漏命中 ≠「无 skill」，含义
对上就路由；② 真·单文件 / 机械 / 一次性小改动走平凡任务豁免、不硬套 skill/流程。裁决：高置信直接路由；
多个合理一次一问；实质功能 / 代码需求先过 Plan Agent 5 条件；dispatch 前过**链路检查**（R1 研究前置-仅
裸奔点 / R2 OD-first / R3 端到端确认，全文 `.claude/skill-os/routing-chain-check.md`）。**边界（乙类不适用）**：记忆检索时机、
模型选档、checkpoint、research 门、observability、Coding Discipline、handoff/双仓一致是**常驻过程纪律、
非路由目标**，靠 hook 确定性或编排层强制，不塞进"按含义路由"（用户显式点名某项仍照做——排除的是当
语义识别目标，不是拒绝执行）。
