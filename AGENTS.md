...
2. Complexity second: 复杂需求 → Plan Agent，不进单个 skill。
3. Ambiguity third: 多候选 → 问用户，不自行判断。
4. Single skill last: 只在高置信时调用 skill。
...
2. **Complexity gate.** If route-guard indicates `PLAN MODE`, read `.claude/agents/plan-agent.md`
   and produce a phase plan before any single skill.
