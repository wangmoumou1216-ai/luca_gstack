...
version: 1.0.0
description: |
  HTML 原型生成与可观测 QA。三种场景行为完全不同：A（新功能，Step0认知门禁+
  framework母版+全量状态）；B（优化现有，区分改动区/保持区+对照截图）；
...
|---|---|---|
| `design_brief` | 已有 `docs/decisions/*-design-brief.md`，且用户要求按交互文档落地 | 否，但 workflow mode 下优先 |
...
| `standalone_brief` | 用户直接描述要做的原型 | 否，但必须补齐最小上下文 |

**Workflow mode**：如果用户明确说“继续流程/进入下一步/按上游产物做”，
则执行对应 workflow gate。

**Standalone mode**：如果用户直接要求做 HTML 原型，允许用 `standalone_brief`
或其它 source_kind 开始；不强制要求补齐 PRD / ux-brainstorm / design-brief。
缺口必须在 `prototype-spec.md` 中声明。
...
>
> A）**新功能设计** — 从零生成，需要选 framework 母版，全量状态
...
**场景A：**
```
□ 读取 framework/README.md（框架使用说明，必读）
...
□ framework/ 目录存在且包含目标母版文件
  → 否：BLOCKED — framework 目录不存在
□ brand-tokens.md 存在
  → 否：BLOCKED — brand-tokens.md 缺失
```

**母版询问（仅当 design-brief 未填母版时触发）：**

AskUserQuestion：

> design-brief 里未记录母版选择。请确认本次原型使用哪个母版？
>
...
不明确时，AskUserQuestion 让用户从 5 个母版或「局部改动/独立组件」中确认。
