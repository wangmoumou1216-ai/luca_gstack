...
prompt, the five review dimensions, the convergence guard, and the finding-classification router.
...
### Dimension 6: AI Native Soundness (conditional — skip if Phase 2.5 landing_judgment is `not_suitable` or absent)
...
<review_findings round="{N}">
  <finding id="F{N}.1">
    <dimension>{Coherence | Feasibility | Scope | Assumption | Handoff | AI_Native}</dimension>
