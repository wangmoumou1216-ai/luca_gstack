...
| U-11 | Codex 侧无法传 per-agent model 参数 / Codex 上 frontmatter inert（故 model-tier 分档不可移植 LOCKS） | 真 Codex 是否暴露 per-agent model dispatch 参数 / 是否消费 SKILL.md frontmatter | Codex 上 spawn 子 agent 尝试传 model 参数；确认 recommended-model frontmatter 是否被读取 | ASSERTED_AS_FACT（AGENTS.md 无 hedge、§4.8 无 blanket UNVERIFIED）→ 见 finding H6-001 | Codex 暴露 per-agent model 参数 / frontmatter 消费 |
