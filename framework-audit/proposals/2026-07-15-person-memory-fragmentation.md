...
> **状态校准（2026-09-06）：CLOSED / SUPERSEDED。** 技术处置与审计链已在 commit `5672fa5` 落地，A5 部署 scope 验证由 `c3fba4b` 闭合；`SC-20260715-005`、`SC-20260715-006` 已拒绝，修正版 `SC-20260716-001` 已晋升。仍保留的边界是 key 静默失效需人工复验及历史孤儿目录，不构成本案技术面未完成。以下正文保留为历史诊断。
...
**2026-07-16 追加登记（红队 R3 指出当日双写漏登，编排 session 72da83e0 补记）：**

| 文件 | 动作 | 说明 |
|---|---|---|
| `candidate_feedback_ant_profile_shadows_max.md` | **已消费**：luca 裁决晋升为 `feedback_symptom-first-before-acting.md`，候选两侧删除 | **数据零丢失**。两侧正文的「同族」交叉链**有意不同**（各链本 store 真实存在的邻居：母版链 `feedback_verify-with-real-evidence-before-reporting`，fork 链 `verify-runtime-not-spec`）→ 2570B vs 2629B 是适配非漂移。红队 R3 把这次重命名误读为「从两库同时蒸发、零告警」——以本登记为准：digest（09:49）只是比重命名（09:45–46）晚 3 分钟 |
| `candidate_feedback_verify-params-before-offering-choices.md` | byte-identical 双写（10:27/10:28） | 同上表原则 |
| `candidate_feedback_verify-in-the-deploy-scope.md` | byte-identical 双写（11:12/11:13） | 同 |
| `candidate_feedback_disclosure-is-not-remediation.md` | byte-identical 双写（11:13） | 同 |

以上 4 项 + 两侧 `MEMORY.md` 索引行 = 2026-07-16 当日写入两个真 person store 的**全部 10 个文件，均为 72da83e0 session 所写**（红队把 09:45–47 的写入归给 522aa1b3 系推断误归，本人一手认领）。v3 的 V0「本 session 零写入」（探针截断 13:00）与 V2 台账「共有 6 / IDENTICAL 4 / DRIFTED 2」因未见此登记而失真——**此登记即修正源：两库在 07-16 之前的自然精确同名重叠仅 2**。

**2026-07-16 R4 终审结束后追加（冻结解除，编排 session 72da83e0 落 3 笔推迟写入）：**

| 文件 | 动作 | 说明 |
|---|---|---|
| `feedback_missing-vote-is-not-a-verdict.md` + 两侧 `MEMORY.md` 各 +1 索引行 | byte-identical 双写 | 信号①：luca 纠正「第四轮没做完，做完」——评审轮因基建故障缺票不算完成轮；缓存回放补票后 exec 视角带回全轮唯一沙箱实跑级新发现 |
| `candidate_feedback_orchestrator-provenance-ledger.md` | byte-identical 双写 | R1-R3 死因的结构教训；R4 内联台账+在飞冻结后 A 类缺陷 5/5 视角确认消失（修法实证有效） |
| `candidate_feedback_verify-params-before-offering-choices.md` | 两侧各追加复发注记 | 当日原样复发：给 luca 的 reject 命令写错脚本（review_candidates→consolidate_memory），交接前 --help 验出 |

R4 期间（约 14:00–17:20）两 store 冻结保持为真——5/5 视角以 `find -newermt` 实测确认台账
10 文件即当日全部写入。本块 3 笔均为 R4 完成后写入。

**2026-07-16 执行登记（luca 解除拍板「那你按照这个执行」；runbook = 同目录 v5-exec，Fable 复审三修复 + opus drill 三项全绿后执行）：**

- **M3-0**：`grep-cjk` 孤儿候选 `cp -n -p` 入 canonical（sha 相等 + mtime 保 07-11，age 5d 如实进治理面）。
- **M3-a**：9 个 fork-only 文件 `cp -n -p` 入 canonical + canonical MEMORY.md append 9 行（索引 27→36）；
  A0/A1/A2/A3-live/A4 当场全绿；fork store 原地冻结未动（终检 A3-live PASS，窗口内零变化）。
