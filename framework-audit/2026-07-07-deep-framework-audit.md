...
母版记录保留（是真实 session）。
**F1-05/F2-07 (P2/P3, DECIDE)** person 层 split-brain：harness 给 fork session 实际注入 fork 自己的
全局目录，而 hook/治理/文档四处硬编码母版目录。需裁决 canonical 目录（推荐：维持「母版目录为
canonical」的文档协议，但把 harness 落盘面也统一过去，或反之——二选一后改 4 处硬编码为单一真值源）。
