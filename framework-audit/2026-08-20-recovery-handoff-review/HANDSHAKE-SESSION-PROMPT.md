...
11. …/framework-audit/2026-08-20-recovery-handoff-review/REVIEW.md   ← 本轮新增，说明 4–10 里
    哪些数字已过期、为什么，以及非负条款的来由
...
**动 project pin / project-scope-guard / PreToolUse 相关任何文件之前**，先读
framework-audit/2026-08-20-routing-steering-handshake/ 下的在飞产物（该工作流在
2026-08-20 19:33 仍在写入，其 Round-2 红队的 5 条 BLOCKER 全部围绕 TURN_ACTIVE / epoch /
lease / 直接调用重放）。三方在改同一套机器；不对齐就动手会再撞一次。
若该工作流仍在飞，把交集路径列为 BLOCKED_DIRTY_OVERLAP 并在人类门报告，不要抢改。

────────────────────────────────────────────────────────
非负条款 —— 安全后置条件（不可协商）
────────────────────────────────────────────────────────

任何 hunk 应用、分支恢复、package 重建之后，**必须当场跑出并留证**：
...
任一不满足 → 判 REVERTS_SECURITY_FIX，回退该次恢复并停在人类门。

来由：forensic 分支上这三个符号计数**全部为 0**。它们分别堵的是 `..` 词法穿越、
软链绕过、变量拼接绕过——三个都在 2026-08-20 由红队**实证复现**过，不是理论风险。
散文写「不要回退安全修复」没有约束力，只有跑得出来的断言有。
