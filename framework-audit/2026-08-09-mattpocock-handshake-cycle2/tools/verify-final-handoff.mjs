...
if (!failures.length) {
  ...
  try {
    const actualHead = execFileSync('git', ['-C', REPO, 'rev-parse', 'HEAD'], { encoding: 'utf8' }).trim();
    if (actualHead !== manifest.frozen_inputs.canonical_head) fail(`canonical HEAD drifted: ${actualHead}`);
