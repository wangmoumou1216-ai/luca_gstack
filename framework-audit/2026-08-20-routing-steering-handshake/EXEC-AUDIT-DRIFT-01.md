...
| R-10 | §7.1 `A-SCOPE-NULL`「否定式与其肯定式孪生必须返回**相同**决策」 | **前半句过度指定，只钉后半句**（「都不得返回任何会改绑定的 `projectAction`」）。`不涉及项目` 本就该被判成框架活、不 gate——那正是 `NEGATED_DOWNSTREAM_SCOPE_RULES` 存在的目的；要求两种极性同决策等于要求删掉 §6 明确保留的那条能力，与同一份计划自相矛盾。实测：修复后肯定式 `NEEDS_CONTEXT`、否定式 `NONE`，**都不改绑定**——安全属性成立而决策本就应当不同。补三条反向对照防过度修复：具名项目仍 gate（SC-20260523-002）、正常新建仍工作、显式「新建项目 X」两种极性行为一致（后者经对照组实测：无框架信号时两极性**同样**建项目，那是用户显式声明项目名，不是作用域否定缺陷） | P4 实测 |

### R-11（P5 执行口径，luca 2026-09-01 批准走 (a)）—— 义务升 `PENDING` 的非语义代理

§4.2 写「升 `PENDING` 的唯一途径 = 下一个**经认证的**人类事件带着**肯定式任务指令**」。
这句在本次范围内**字面不可实现**：「经认证」是 E3 的惰性认证层（已按会审裁决拆出），
「肯定式」是语义判断，而 §3.3 刚用六轮红队论证 hook 判不了中文否定——让 hook 判「肯定式」
等于从另一个门把删掉的机制引回来。

**取非语义代理**：Claude 侧 `UserPromptSubmit` 只在真实用户回合触发，「认证」在结构上已满足；
「肯定式任务指令」取「下一个人类回合**再次产出 E2 信号**」（用户重述了同一类诉求），
其余任何事件按 §4.2 原文直接删除 `SIGNAL_UNCONFIRMED`。
Codex 侧的加固正是 E3 要补的那一层——**E3 落地后只需把这一处换成真认证，状态机与注入面不必改**。

代价面很小：义务按 08-31 裁决**不拦任何东西**，误升的唯一后果是多一行 hint，且 20 轮封顶自动
`CANCELLED`。取消语义沿用本文件既有的「整句锚定」手法（`不用了`/`算了`/`取消` 等整句形态），
不新建词表机制；判错的唯一后果是少一行提醒。
