...
**✅ never-switch 记忆错位——已修复（2026-07-09，memory-eval session 处理，无需你再动）**：
- person 层记忆 `feedback_never-switch-parallel-session-projects.md`（luca 标"严重问题"）曾错位在
  fork auto-memory 目录，母版权威目录缺失（M1 割裂复发）。
- **已 union 迁入母版权威目录 `-Users-luca-Desktop-luca-gstack/memory/`（逐字复制）+ 母版 MEMORY.md
  加索引行；fork 目录该文件已删、fork MEMORY.md 索引改标"已迁母版"。** 母版启动 session 现可见此规则。
