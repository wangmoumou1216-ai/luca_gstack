...
## C6. session-isolation（方案A）依赖 + never-switch 记忆错位（新追溯）

**为什么这段重要**：我的「绑定即注入 + 边界条款」和 session-isolation（方案A）是**咬合**的——
边界条款里"switch 翻全局软链、已 pin session 免疫、未 pin 读 session 被拖走、方案A 已移除漂移告警"
这些论断，全部依赖方案A 的机制。port 前必须确认母版有方案A。

**方案A 同步状态（已核验）**：
- fork：`2468e88`（命名即切换）+ `04a5faf`（方案A 会话级隔离）+ `e4a9002`（refine 放宽未绑定读/收紧认领）
- 母版：`def1fc0`（方案A）+ `b46c8bf`（refine）——已 port 到位（审计 F5-06/07 记录的"当场复发第二例"就是这次 port）
- 核心文件 `.claude/hooks/project-scope-guard.mjs` **两仓逐字一致（0 差异）**。
- **结论：母版已具备方案A，C5 的绑定即注入 hunk 可安全落母版**（它引用的机制母版都有）。

**⚠️ never-switch 记忆错位（M1 割裂复发，需处理）**：
- person 层记忆 `feedback_never-switch-parallel-session-projects.md`（luca 标"严重问题"，内容=meta/审计
  session 永不 `project.sh switch`）**当前在 fork auto-memory 目录**
  （`~/.claude/projects/-Users-luca-Desktop----muse-gstack/memory/`），
  **母版权威目录（`-Users-luca-Desktop-luca-gstack/memory/`）没有它**。
- 按 M1 定案（person canonical=母版目录），这条**该在母版目录**，否则母版启动的 session 看不到——
  正是 M1 要根除的割裂又发生一次（写入方没走 `reference_person-memory-canonical-dir` 的指向）。
- **处理**：把 `feedback_never-switch-parallel-session-projects.md` union 迁入母版权威目录
  + 母版 `MEMORY.md` 加索引行（这是仓外 `~/.claude/` 操作，不进 git 仓，与母版 port 独立）。
  它是**通用行为规则**（非 fork 语境专属），迁 canonical 后 fork 目录可删该条。
