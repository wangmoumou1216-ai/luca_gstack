...
导致 `password`→docx 这类误伤。已修 11 项（全部经实测 + 全量回归，含独立复审打回后补修的 3 项）；原先留给裁决的 3 类里，2 类同日修掉，只剩 1 项**行为决策**（是否给 careful 接线）留 luca。
...
**留给 luca 的一条**：本轮 10 条晋升由我以「caller 自报授权」的形态放行，而其中
`SC-20260830-001` 写的正是「caller 自报 authority 只能作审计、不能充当授权边界」。授权是真的
（luca 原话「你来决策」），但形态自指。是否复核这批晋升或由 luca 重签，留他定；未自行回滚。
