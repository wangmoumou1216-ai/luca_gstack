...
7. magicpath        → **首选**：需求描述 → React canvas 组件
   /html-prototype → （备选）生成 HTML 原型（MagicPath 不可用时）
...
7. magicpath        → **首选**：需求描述 → React canvas 组件
   /html-prototype → （备选）生成改版原型（MagicPath 不可用时）
...
4. magicpath        → **首选**：需求描述 → React canvas 组件
   /html-prototype → （备选）按评审清单逐条生成改版原型（MagicPath 不可用时）
   /figma-demo     → （备选）基于 Figma 截图/链接和口述生成演示 Demo
...
8. magicpath        → **首选**：需求描述 → React canvas 组件（含 Agent 专有状态）
   /html-prototype → （备选）生成原型（MagicPath 不可用时）
...
magicpath      A B C D **设计产出首选**：需求描述 → React canvas 组件
               输入模式：standalone 或 workflow。standalone 可直接描述界面；
               workflow 必须消费 design-brief 的 Design Generation Packet
               说明：有 MagicPath canvas 环境时优先使用；不可用时退回 /html-prototype

/html-prototype A B C  生成可在浏览器查看的 HTML 原型（MagicPath 不可用时备选）
...
/figma-layer   A   C   把 HTML 原型还原到 Figma，作为交付稿
               输入模式：standalone 转换工具或 workflow 下游。需要 /html-prototype
               或 /figma-demo 的 index.html + prototype-spec
