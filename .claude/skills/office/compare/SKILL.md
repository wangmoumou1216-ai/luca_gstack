...
**不修改输入文件。** 只读输入、分析、报告；对比报告本身写入 `docs/decisions/`（见 Phase 4），不改动被对比的两个文件。

---

## 输入模式

> compare 为隐藏 skill（2026-07-03 零调用降级），无斜杠命令、不在 skill-routing-map；按名调用——用 Skill 工具调 `compare`，或 agent 读取本 SKILL.md 执行。下列 `compare <args>` 是参数形态说明，**不是斜杠命令**。
