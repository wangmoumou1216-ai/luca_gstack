...
> **⚠️ L3/L4 颜色的无障碍注记（2026-07-22 实测，默认值未改）：** `.text-l3` / `.text-l4` 用的
> `#91959E` 在白底上仅 **3.00:1** —— 过非文本 3:1，**过不了正文 4.5:1**；而它们承载的是字段名与
> 时间戳等**真实文字**。面向欧美 / 受 EAA·ADA 影响的项目必须改用同色相压深的 **`#6E727B`（4.82:1）**，
> `#91959E` 只留给非文字装饰。这与 §六「白字亮底」是同一问题的两面，完整实测表见
> `brand-tokens.md` §无障碍合规注记。**注意本节「字段名比字段值浅」的强制规则仍成立**——
> `#6E727B` 依然浅于 `#181C25`，替换不破坏该规则。
...
> **⚠️ 无障碍合规注记（2026-07-22 实测；上表默认值未改，覆盖责任在项目侧）**
> 上表「主操作 `bg #FF8000 / text #FFF`」与「危险操作 `bg #FF522A / text #FFF`」，以及 §二的
> `--primary-foreground: #FFFFFF`，**实测均不满足 WCAG 2.x AA**：`#FF8000`+白字 = **2.52:1**
> （连非文本 3:1 都不过）、`#FF522A`+白字 = 3.23:1（过 3:1，不过正文 4.5:1）。
> 同源问题覆盖整个功能色板（success/warning/info 白字为 1.96 / 2.58 / 2.87），完整实测表见
> `brand-tokens.md` §无障碍合规注记。
> **目标市场为欧美、或受 EAA/ADA 影响、或客户合同写明无障碍要求的项目，必须覆盖前景色**，
> 两条已验算的解：① 保留底色、前景改近黑 `#181C25`（`#FF8000` → 6.77:1，品牌色零改动，
> 与 Carbon/Polaris 处理亮暖色的做法一致）② 底色压深、维持白字（橙 → `#B85C00`，4.60:1）。
> 默认值不改的理由：本色板为国内市场语境所建，改默认会动所有既有原型观感——**不是当初做错，
> 是国际化让它成为承重项**。溯源：语义候选 `SC-20260722-003`。
