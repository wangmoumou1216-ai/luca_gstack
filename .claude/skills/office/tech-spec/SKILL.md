...
context-cost:
  ...
  recommended-model: core-execution  # 2026-07-10 新增声明：工程合同=承重执行档（此前静默吃默认档）
---
...
- **开发交接四维附加字段（optional，源=原型 prototype-spec.md 的"开发交接补全"节）**：`CMP-NNN`
...
7. tech-spec 是工程合同，不是设计文档；不得包含 UI 布局描述（**开发交接四维属实现规格——断点维=可测 reflow 行为契约、绑 CMP-NNN 作测试准则，非静态布局复述；见 Phase 3 附加字段**）
