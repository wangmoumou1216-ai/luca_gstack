...
不执行实现——我的产出是两份markdown文档：设计方案 + 交互架构。
...
线性流程（无分支）：ux-research*.md（或设计想法）→ Phase 0 → 1 → 2 → 2.5 → 3 → 3.6 → 4 → 5 → 6 → 7 → 8
...
**范式转变约束（强制）：** 至少一个approach必须满足 CRITICAL RULE 7 定义的范式转变约束
（移除≥1执行步 + 新增≥1判断辅助）；此处仅作勾选，阈值与完整定义以 RULE 7 为唯一真值源。
