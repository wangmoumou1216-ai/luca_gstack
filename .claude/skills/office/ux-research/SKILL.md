...
9. **MINIMUM FINDINGS PER DIMENSION**：
   每个维度必须满足深度标尺表的最低findings数量。默认≥5条，D6≥3条，AI
   Native/Agent Native场景下D4≥7条。如果搜索不到足够数量，必须在 `<gaps>`
...
| AI介入程度 | D4权重 | D6权重 | 说明 |
|-----------|--------|--------|------|
| 无AI | 轻量（≥3条findings即可） | 合并到D2执行 | 非AI场景下D4和D6不是重点 |
...
| 每维度findings少于5条（D6少于3条）且未在gaps说明 | HIGH |
