...
这是本 skill 的原创核心（外部无成熟先例）。**判据的唯一真值源 = `references/recovery.md`「完整性门判据」；下方为速查副本，与其冲突时以 recovery.md 为准，改判据须同步两处。** 判据：
...
**取全文优先级与 `atomic` 解析配方的唯一真值源 = `references/recovery.md`「FxTwitter 恢复配方」；下方为速查副本，与其冲突时以 recovery.md 为准，改配方须同步两处。**
