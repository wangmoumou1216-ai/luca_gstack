...
context-cost:
  ...
  shared-refs: [none]
  recommended-model: sonnet  # 基于checklist审查
---
...
- 任何节有未通过项 → 输出 `❌ HANDOFF REVIEW: FAILED`，列出需要修正的具体项

**【H-04 修复】迭代计数与 workflow-state 写入：**

上方「通过判断」执行后，Claude 确定结果（PASSED 或 FAILED），然后执行：
