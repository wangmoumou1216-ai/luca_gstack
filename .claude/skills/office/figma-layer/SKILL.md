...
```
品牌色 Variables：--primary #FF8000 · --ring #FF8000
  --primary-foreground：**必须显式声明，不得默认白色**（白字在 #FF8000 上实测 2.52:1，不满足 WCAG AA
  正文 4.5:1，连非文本 3:1 都不过）。二选一按项目市场取：近黑 #181C25（6.27:1 ✓，面向欧美/受
  EAA·ADA 影响时**必须**用这条）／ 白色 #FFFFFF（2.52:1 ✗，仅国内且项目显式接受风险）。
  上游 design-brief 的 Packet 若已声明前景色，以它为准。
文字色 Variables：主文字 #181C25 · 次要文字 #91959E
  ⚠️ #91959E 白底仅 **3.00:1**——过非文本 3:1，**过不了正文 4.5:1**。它只可用于**非文字装饰**
  （分隔点/图标/滚动条）；凡承载**字段名/时间戳等真实文字**的槽位，改用同色相压深的 **#6E727B（4.82:1）**。
  合规背景见 `brand-tokens.md` §无障碍合规注记。
