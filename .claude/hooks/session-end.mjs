...
//  · identity pin 刻意保留：SessionEnd 无 generation snapshot，不能证明自己拥有当前 state
// 目的（红队 H2）：优雅退出时立即回收本 session 的临时活跃信号，把"僵尸粘住窗口"从 15min GC
...
try {
  ...
  // Deliberately preserve `.session-project-<sid>`. SessionEnd payload has no
  // generation/expected-state snapshot, so a delayed/replayed End cannot prove
  // that the current identity belongs to it. Explicit deactivate owns removal.
} catch { }
process.exit(0);
