...
function validateTicket(ticket) {
  exactKeys(ticket, [
    ...
    'prompt_sha256', 'nonce', 'created_at',
  ], 'correction ticket');
...
function ticketFromBinding({ sessionId, eventId, promptSha256, now = new Date().toISOString() }) {
...
export function issueCorrectionTicket({ root, sessionId, eventId, prompt, promptSha256, now }) {
  ...
  const promptHash = promptSha256 || sha256(Buffer.from(String(prompt), 'utf8'));
  const ticket = ticketFromBinding({ sessionId, eventId, promptSha256: promptHash, now });
...
function validateRequest(request) {
  exactKeys(request, [
    'schema_version', 'ticket_id', 'session_id', 'event_id', 'prompt_sha256', 'nonce',
...
function validateReceiptShape(receipt) {
  exactKeys(receipt, [
    ...
    'session_id', 'event_id', 'prompt_sha256', 'nonce', 'attribution_level',
...
function verifyReceiptAgainstTicket({ root, receipt, ticketRecord }) {
  ...
  const reverified = receipt.evidence.map((entry, index) => {
    exactKeys(entry, ['kind', 'verifier', 'path', 'selector', 'claim_sha256', 'artifacts'], `receipt evidence[${index}]`);
...
export function closeCorrectionTicket({ root, request, baselineCounts, now = new Date().toISOString() }) {
  const { requiredKinds } = validateRequest(request);
  validDate(now, 'receipt closed_at');
  ...
  try {
    if (existsSync(paths.completion)) {
      const current = validateCompletion(JSON.parse(readFileSync(paths.completion, 'utf8')));
    ...
    if (existsSync(receiptPath)) {
      const existingRaw = readFileSync(receiptPath);
    ...
    const activeReadback = readFileSync(paths.active);
...
export function readCurrentCorrectionReceipt({ root, sessionId }) {
  ...
  readExistingState(root, sessionId);
  let completion;
  try { completion = validateCompletion(JSON.parse(readFileSync(paths.completion, 'utf8'))); } catch (error) {
