...
**必须知道的操作坑：`scripts/luca-open.sh` 只接受本地文件绝对路径，不接受 URL。**
直接传 Figma URL 会 `cd: No such file or directory` 报错退出。`scripts/luca-sidebar.sh` 是**只读**的
（只有 `meta` / `capture` 两个模式，用于问当前开了什么），**没有**把 URL 推进侧栏的写接口。
