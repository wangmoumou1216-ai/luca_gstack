...
> `.claude/hooks/session-sync.mjs` 在默认 Stop 路径只幂等写 `pending-extraction`，不打断中间对话；
> 显式设置 `SESSION_SYNC_FORCE_ON_STOP=1` 时才注入「四信号速记 + 本文件路径」短指针
> （HOOK-007 锁定 reason ≤900 字符，不复制全文）；裁决细则以本文件为准。
...
【时机】person/项目层由 Stop 写 pending、下次启动统一提醒裁决；对话中途仅信号①允许即写；semantic 候选提议不受此限
...
- session-sync 显式强制模式的 reason 只放四信号速记与本文件指针（≤900 字符，HOOK-007 锁定），不得回归整段复制。
