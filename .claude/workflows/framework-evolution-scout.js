...
function verifyPrompt(c) {
  ...
  return 'You are a SKEPTICAL, ADVERSARIAL verifier. Find reasons to REJECT, not to praise. When uncertain, score LOW and FAIL hard gates. Use REAL tool output only.\n\n' +
    ...
    '  compatibility (install-type: valid frontmatter, droppable into ~/.claude/skills or .claude/, no trigger collision; pattern-type: the pattern is portable without dragging a heavy runtime — if it REQUIRES importing a whole framework, FAIL),\n' +
