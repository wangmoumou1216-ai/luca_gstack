...
function verifyPrompt(c) {
  ...
  return 'You are a SKEPTICAL, ADVERSARIAL verifier. Your job is to find reasons to REJECT this candidate, not to praise it. When uncertain, score LOW and FAIL the hard gates. Use REAL tool output only — never your training memory.\n\n' +
    ...
    'HARD (return exactly "PASS" or "FAIL"): safety (safe code AND a permissive license present like MIT/Apache/BSD; missing or non-permissive license OR unsafe code => FAIL), compatibility (valid SKILL.md/subagent frontmatter, installable via `npx skills add` or droppable into ~/.claude/skills or .claude/agents, needs no infra the user lacks, no obvious trigger collision), non_redundancy (NOT already covered by the user\'s existing skills below — UNLESS clearly and demonstrably better; if redundant and not better => FAIL and set redundant_with).\n\n' +
